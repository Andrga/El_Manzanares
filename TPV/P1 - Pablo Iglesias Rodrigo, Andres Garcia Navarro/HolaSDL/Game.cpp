...
// Struct para inicializar las texturas
struct Textura
{
	string name;
	int rows;
...
// Inicializacion de caracteristicas de texturas para creat las texturas
...
// Clase error explicada en las diapositivas de clase
class Error {
protected:
	string mensaje;
public:
...
void Game::render() {
	//Contador para el cambio de animaci�n
	if (tiempoTrans == 2500)
	{
		tiempoTrans = 0;
		for (const auto e : aliens)
...
{
	lasers.push_back(new Laser(position, velocidadLaser, alien, this, renderer)); // A�adimos el laser a la lista de lasers.
}
...
void Game::colDetection(Laser* laser) {
	...
	//deteccion de colision del laser con los aliens, si el laser proviene del ca�on
	...
	//deteccion de colision del laser con el ca�on si proviene de los aliens
